...
### Required
- Web analysis report: read from `outputs/{slug}/details/web-analysis-report.md`
- Branding Visual Guide (full): read from `outputs/{slug}/deliverable/{slug}-branding-visual-guide.md`
  (Full guide required — report generator needs all design tokens, typography, colors to style the HTML)

### Required from runtime context
- clientName
- reportTitle
- preparedBy
- date
...
## Output note
This step produces a client-facing HTML report. No slim handoff written.
Save to: `outputs/{slug}/deliverable/{slug}-web-analysis.html`
