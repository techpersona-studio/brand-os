...
1. **Business Persona** (required) — includes Section 5: Visual & Styling Cues, which already consolidates all visual signals from both researchers. This is your primary and sufficient source of truth. Do not ask for separate researcher outputs.
2. Optional **logo file or brand asset images** — use only to observe color, mark style, and typographic feel. Do not infer strategy from aesthetics alone.
3. Optional **client comments or visual preferences** — use to refine direction, not override confirmed strategy.

Do not request or depend on raw researcher outputs. Everything needed is in the Business Persona.
...
Render a table with one row per item below and a Locked / Tentative / Not locked status column:
...
Then write a one-line overall status that matches the Generator readiness field in Section 12.
...
* Primary reading font direction:
* Accent / display font direction:
* Tone:
* Hierarchy:
* Heading behavior:
* Body behavior:
...
{{BUSINESS_PERSONA}}
{{LOGO_OR_BRAND_ASSET_IMAGES}}
{{CLIENT_COMMENTS_OR_PREFERENCES}}
